...
class podcasts:
  ...
  def AppendRSS(self,name):
    ...
    appendable = self.Casts.select(self.cdb,['name','url'],['^'+name+'$',''],['name','url'])
        ...
        if urlgood:
          ...
          for entry in rssfile.entries:
            ...
            if NewEntryCut not in ExistingEntries:
              # get lock and add :D
              self.RssLock.acquire(blocking=True)
  ...
  def AddCast(self,name,url,update):
    ...
    if not checkurl(url):
      ...
      self.CastsLock.acquire(blocking=True)
  ...
  def EditCast(self,oldname,newname,url,update):
    ...
    if newname not in self.AllCasts or newname == oldname:
      self.CastLock.acquire(blocking=True)
